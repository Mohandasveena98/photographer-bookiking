			...
			<div class="row w3ls_banner_bottom_grids">
				...
				<div class="col-lg-5 mt-lg-0 mt-5 wthree_mail_pos">
    				...
    				if(isset($_POST['change']))
    				{
    					...
    					$qry=mysqli_query($con,"SELECT * from photographer where phid='$phid' and email='$email'") or die(mysqli_error($con));
    					...
    					if($rows==1)
    					{
    						if($npwd==$cpwd)
    						{
    							...
    							if($sql)
                					{
                						echo "<script>alert('Password is successfully changed'); window.location='index.php';</script>";
                					}
                					else{
