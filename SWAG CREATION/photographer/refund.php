...
<?php include 'con_db.php';
	include 'session.php';
	$id=$_GET['id'];
	$sql=mysqli_query($con,"SELECT * FROM bookings,user where phid='$phid' and bookings.uid=user.uid and bid='$id'");
	$row=mysqli_fetch_array($sql);
	$prof=$row['advance']*0.10;
	$ref=$row['advance'] -$prof;
 ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<!-- //breadcrumbs -->
	<div id="mail" class="banner-bottom mail contact pt-5">
		...
		<h3 class="heading text-center mb-sm-5 mb-4">Refund Here</h3>
			<div class="row">
                     ...
                     <div class="col-md-4"></div>
                        ...
                        <form action="" method="POST">
                            ...
                            <p style="color: white;">Amount to Refund</p>
                            <input type="text" name="refund_amt" readonly="" value="<?php echo $ref; ?>" class="form-control">
                            ...
                            <input type="password" name="cvv" placeholder="CVV" pattern="^[0-9]{3}" title="only 3 digits" class="form-control float-right" style="width: 80px;"><br><br>
                        ...
                        <?php
                          include('con_db.php');
                          if(isset($_POST['pay'])){
                             $refund=mysqli_real_escape_string($con,$_POST['refund_amt']);
                              $qry_insert=mysqli_query($con,"UPDATE bookings SET refund_status='refunded',refund_amt='$refund' WHERE `bid`='$id'") or die(mysqli_error($con));
                                if ($qry_insert) 
                                      {
                                          echo "<script>alert('Refund Done'); 
