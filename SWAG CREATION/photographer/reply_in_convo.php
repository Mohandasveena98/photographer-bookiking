	...
	<div id="mail" class="banner-bottom mail contact pt-5">
		<div class="container">
                ...
                <form action="" method="POST">
                    <textarea id="form_message" name="q_content" class="form-control irs-textarea" rows="8" style="width: 600px;" placeholder="Type Your Reply*" required="required" data-error="Message is required."></textarea><br><br>
                    <button class="btn btn-info" name="send" type="submit">Reply</button>
                </form>
                <?php  
                ...
                if(isset($_POST['send'])){
                    ...
                    $qry_insert=mysqli_query($con,"INSERT INTO `enquiry`(`enqid`, `uid`, `phid`, `enquiry`, `sentdate`, `response`, `rdate`, `status`) VALUES ('','$uid','$phid','$q_content',now(),'','','pg_sent')") or die(mysqli_error($con));
                    
                      if ($qry_insert) 
                            {
                                echo "<script>alert('Query sent'); 
                                window.location='viewconvo.php?uid=".$uid."'; 
                                </script>";
                            }
                            else
                            {
                                echo "<script>alert('Failed');  window.location='viewconvo.php?uid=".$uid."'; </script>";
                            }
                }
            ?>
