...
<!-- //breadcrumbs -->
	<div id="mail" class="banner-bottom mail contact pt-5">
		...
		<h3 class="heading text-center mb-sm-5 mb-4">Gallery</h3>
			<div class="row w3ls_banner_bottom_grids">
				...
				<div class="col-lg-10 mt-lg-0 mt-5 wthree_mail_pos">
					<table class="table table-bordered">
						...
						<tbody>
							<?php $i=1;
								$sql=mysqli_query($con,"SELECT * FROM gallery where phid='$phid' ORDER BY gtitle ASC") or die(mysqli_error($con));
								while($row=mysqli_fetch_array($sql)){
							?>
								<tr>
								<td><a href="del_gallery.php?gid=<?php echo $row['gid']; ?>" class="btn btn-danger" onclick="return confirm('Sure to delete it?'); "><i class="fa fa-trash"></i></a></td>
								<td><?php echo $i++; ?></td>
								<td><?php echo $row['gtitle']; ?></td>
								<td><?php 
										$splittedstring=explode(",",$row['images']);
										foreach ($splittedstring as $key => $value) 
										{
												
										?>
										<img src="images/<?php echo $value ?>" style="width: 70px; height: 70px;">			
										<?php
										}
			 							?></td>
