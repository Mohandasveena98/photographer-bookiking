                    ...
                    <div class="irs-account-col">
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            <tr>
                             ...
                             <?php include('con_db.php');
                            $a=1;
                            $sql=mysqli_query($con,"SELECT DISTINCT enquiry.`uid`,sentdate,fullname,contact from enquiry,user where enquiry.uid=user.uid and enquiry.`phid`='$phid' group by enquiry.`uid`") or die(mysqli_error($con));
                            while($row=mysqli_fetch_array($sql)) {
                             ...
                             <tr>
                                 ...
                                 <td><?php echo date('d-m-Y',strtotime($row['sentdate'])); ?></td>
                                 <td><?php $uid=$row['uid'];
                                    $qry=mysqli_query($con,"SELECT count(*) from enquiry where phid='$phid' and uid='$uid'");
                                    $rcnt=mysqli_fetch_array($qry);
                                 echo $rcnt[0]. ' Conversation'; ?></td>
                                 <td><a href="viewconvo.php?uid=<?php echo $row['uid']; ?>" class="btn btn-info"><i class="fa fa-eye"></i></a></td>
                             </tr>
                          <?php  } ?>
                        </table>
                    </div>
