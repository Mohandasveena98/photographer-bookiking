...
<!-- //breadcrumbs -->
	<div id="mail" class="banner-bottom mail contact pt-5">
		...
		<h3 class="heading text-center mb-sm-5 mb-4">Payment Details</h3>
			<div class="row w3ls_banner_bottom_grids">
				<div class="col-lg-12 mt-lg-0 mt-5 wthree_mail_pos">
					<table class="table table-bordered table-striped table-hover">
							...
							<tbody>
								<?php $i=1;
								$sql=mysqli_query($con,"SELECT * FROM bookings,photographer where uid='$uid' and bookings.phid=photographer.phid");
								 ...
								 <tr>
								 	...
								 	<td><a href="bill.php?id=<?php echo $row['bid']; ?>" class="btn btn-success" title="Bill"><i class="fa fa-print"></i></a></td>
								 	<td>
								 	 <?php $ps=$row['apaystatus']; $rst=$row['refund_status'];
                                                 if($ps=='approved' && $rst=='no')
                                                 {
                                            ?>
                                            <a href="pay_adv.php?id=<?php echo $row['bid']; ?>" class="btn btn-warning" title="Debit Card">Pay Advance</a>
                                           <?php }elseif($ps=='adv_paid' && $rst=='no')
                                                  { ?>
                                           <span class="badge badge-danger">Advance Paid. Pending- Rs: <?php echo $row['bcharges']-$row['advance'];?></span>
										   <a href="pay.php?id=<?php echo $row['bid']; ?>" class="badge badge-danger" title="Debit Card">Pay</a>

                                           <?php }elseif($ps=='' && $rst=='no')
                                                  { ?>
                                           <span class="badge badge-info">Not Yet Approved</span>
										   <?php }elseif($ps=='paid' && $rst=='no')
                                                  { ?>
											 <span class="badge badge-danger">Full amount payed </span>
                                           <?php }?>

										  
                                       <?php $rs=$row['refund_status'];
                                                 if($rs=='refunded')
                                                 {
                                            ?>
                                           <span class="badge badge-primary">Refunded Amount Rs: <?php echo $row['refund_amt'];?></span>
                                           <?php }?>
                                      </td>
								 </tr>
								 <?php } ?>
