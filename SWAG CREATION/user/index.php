...
<!-- //breadcrumbs -->
	<div id="mail" class="banner-bottom mail contact pt-5">
		...
		<h3 class="heading text-center mb-sm-5 mb-4">User Login</h3>
...
<?php 
		if(isset($_POST['login']))
		{
			...
			$sql=mysqli_query($con,"SELECT * FROM user WHERE email='$email' and pwd='$pwd'");
			$row=mysqli_num_rows($sql);
			$read=mysqli_fetch_array($sql);
			if($row==1)
			{
				session_start();
				$_SESSION['fullname']=$read['fullname'];
				$_SESSION['uid']=$read['uid'];
				echo '<script>alert("Welcome '.$_SESSION['fullname'].' "); window.location="home.php"; </script>';
