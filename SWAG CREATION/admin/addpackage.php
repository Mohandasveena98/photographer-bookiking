				...
				<div class="col-lg-5 mt-lg-0 mt-5 wthree_mail_pos">
					<form action="#" method="post">
						<div class="w3_agileits_contact_left">
							<input type="text" name="pname" autocomplete="off" pattern="[A-Za-z\s]+" title="letters only" placeholder="Enter a package name" required="">
							<input type="text" name="val" autocomplete="off" pattern="^[1-9]{1,2}$" title="2 digit number only" placeholder="Set package validity" required="">
							<input type="text" name="pamt" autocomplete="off" pattern="^[0-9]{3,4}$" title="1 to 4 digit number only"  placeholder="Package Amount" required="">
    				...
    				if(isset($_POST['save']))
    				{
    					...
    					$qry=mysqli_query($con,"SELECT * from packages where pname='$pname'") or die(mysqli_error($con));
    					$rows=mysqli_num_rows($qry);
    					if($rows<=0)
    					{
    						
    							$sql=mysqli_query($con,"INSERT INTO `packages`(`pid`, `pname`, `validity`, `pamt`) VALUES ('','$pname','$val','$pamt')") or die(mysqli_error($con));
    							if($sql)
                					{
                						echo "<script>alert('Package Inserted successful'); window.location='addpackage.php';</script>";
                					}
                					else{
