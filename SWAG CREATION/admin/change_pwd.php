			...
			<div class="row w3ls_banner_bottom_grids">
				...
				<div class="col-lg-5 mt-lg-0 mt-5 wthree_mail_pos">
    				...
    				if(isset($_POST['change']))
    				{
    					...
    					$qry=mysqli_query($con,"SELECT * from admin where username='$u_name' and password='$opwd'") or die(mysqli_error($con));
    					$rows=mysqli_num_rows($qry);
    					if($rows==1)
    					{
    						if($npwd==$cpwd)
    						{
    							$sql=mysqli_query($con,"UPDATE admin set password='$npwd' where username='$u_name'") or die(mysqli_error($con));
    							if($sql)
                					{
                						echo "<script>alert('Password is successfully changed'); window.location='adminhome.php';</script>";
                					}
                					else{
