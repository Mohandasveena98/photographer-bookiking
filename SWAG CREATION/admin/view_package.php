			...
			<div class="row w3ls_banner_bottom_grids">
				...
				<div class="col-lg-6 mt-lg-0 mt-5 wthree_mail_pos">
					<table class="table table-bordered">
						...
						<tbody>
							<?php $i=1;
								$sql=mysqli_query($con,"SELECT * FROM packages ORDER BY pname ASC") or die(mysqli_error($con));
								while($row=mysqli_fetch_array($sql)){
							?>
								<tr>
								<td><a href="up_pack.php?pid=<?php echo $row['pid']; ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a> 
								<?php $pid=$row['pid'];
								$sq=mysqli_query($con,"SELECT * FROM subscription WHERE pid='$pid'");
								$rc=mysqli_num_rows($sq);
								if($rc <= 0)
								{
								?>
								<a href="del_pack.php?pid=<?php echo $row['pid']; ?>" class="btn btn-danger" onclick="return confirm('Sure to delete it?'); "><i class="fa fa-trash"></i></a>
								<?php } ?>
								</td>
								<td><?php echo $i++; ?></td>
								<td><?php echo $row['pname']; ?></td>
								<td><?php echo $row['validity'].' months'; ?></td>
								<td><?php echo 'Rs '.$row['pamt']; ?></td>
								</tr>
							<?php } ?>
						</tbody>
					</table>
					<form action="" method="POST">
					<button type="submit" name="Add_package" class="btn btn-warning">Add Package</button>
					</form>
					<?php
						 if(isset($_POST['Add_package'])){
							echo "<script>alert('addpackage'); 
